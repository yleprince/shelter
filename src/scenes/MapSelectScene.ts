...
import { GAME_WIDTH, GRID_COLS, GRID_ROWS, HUD_ROWS, TILE_SIZE } from '../config';
...
export class MapSelectScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    MAPS.forEach((map, i) => this.addCard(map, left + i * (CARD_WIDTH + CARD_GAP), 190, i + 1));
  ...
  private addCard(map: MapDefinition, x: number, y: number, hotkey: number): void {
    const previewWidth = GRID_COLS * PREVIEW_TILE;
    const previewHeight = (GRID_ROWS - HUD_ROWS) * PREVIEW_TILE;
    const cardHeight = previewHeight + 180;
    const bg = this.add
      .rectangle(x, y, CARD_WIDTH, cardHeight, 0x1c1a16)
      .setOrigin(0)
      .setStrokeStyle(2, 0x6b5b45)
      .setInteractive({ useHandCursor: true });

    const cx = x + CARD_WIDTH / 2;
    this.add.text(cx, y + 24, `${hotkey}. ${map.name}`, { ...TEXT_STYLE, fontSize: '24px' }).setOrigin(0.5);
    this.add
    ...
    const start = () => this.scene.start('GameScene', { mapId: map.id } satisfies GameSceneData);
    bg.on(Phaser.Input.Events.GAMEOBJECT_POINTER_OVER, () => bg.setStrokeStyle(2, 0xffd166));
    bg.on(Phaser.Input.Events.GAMEOBJECT_POINTER_OUT, () => bg.setStrokeStyle(2, 0x6b5b45));
    bg.on(Phaser.Input.Events.GAMEOBJECT_POINTER_DOWN, start);
    this.input.keyboard?.once(`keydown-${['ONE', 'TWO', 'THREE'][hotkey - 1]}`, start);
  }
  ...
  private drawPreview(map: MapDefinition, left: number, top: number): void {
    ...
    g.fillStyle(0x4a4232).fillRect(left, top, GRID_COLS * PREVIEW_TILE, (GRID_ROWS - HUD_ROWS) * PREVIEW_TILE);
    for (let row = HUD_ROWS; row < GRID_ROWS; row++) {
